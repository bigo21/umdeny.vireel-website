...
function Brand() {
  return (
    <div className="brand">
      <img src={logoLight} alt="VIREEL" className="brand-logo" />
    </div>
  );
}
    ...
    <div className="nav-dropdown">
      {items.map((item, i) => {
        ...
        return available ? (
          <Link
            key={i}
            to="/formation-bourse-bvmac"
            className="nav-dropdown-item"
          >
            <span className="nav-dropdown-icon">{item.icon}</span>
            <span className="nav-dropdown-label">{item.label}</span>
            <span className="nav-dropdown-badge nav-dropdown-badge--available">
              {item.status}
            </span>
          </Link>
        ) : (
          <a
            key={i}
            href="#formations"
            className="nav-dropdown-item nav-dropdown-item--soon"
          >
            <span className="nav-dropdown-icon">{item.icon}</span>
            <span className="nav-dropdown-label">{item.label}</span>
            <span className="nav-dropdown-badge">{item.status}</span>
          </a>
        );
      })}
    </div>
            ...
            <nav className="nav-drawer-links">
              ...
              <div className="nav-drawer-formations">
                {c.nav.menu.map((item, i) => {
                  const available = item.status === 'disponible' || item.status === 'available';
                  return available ? (
                    <Link key={i} to="/formation-bourse-bvmac" className="nav-drawer-formation" onClick={() => setDrawerOpen(false)}>
                      <span>{item.icon} {item.label}</span>
                      <span className="nav-dropdown-badge nav-dropdown-badge--available">{item.status}</span>
                    </Link>
                  ) : (
                    <a key={i} href="#formations" className="nav-drawer-formation nav-drawer-formation--soon" onClick={() => setDrawerOpen(false)}>
                      <span>{item.icon} {item.label}</span>
                      <span className="nav-dropdown-badge">{item.status}</span>
                    </a>
                  );
        ...
        <div className="formation-bourse">
          ...
          <div className="formation-bourse-right">
            ...
            <div className="form-ctas">
              <Link to="/formation-bourse-bvmac" className="btn btn-red">{bourse.cta} <span className="arr">→</span></Link>
        ...
        <div className="footer-top">
          <div>
            <div className="footer-brand">
              <img src={logoDark} alt="VIREEL" className="brand-logo brand-logo-footer" />
            </div>
            <p
              style={{
                maxWidth: 320,
